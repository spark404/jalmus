...
public class Piano {
    ...
    public Piano(int l, int m) {
        ...
        whiteKeys.add(new net.jalmus.Key(7 * octavanumber * kw + marge + 3, ypos, kw, kh, octavanumber * 12 + transpose, 0));
    ...
    public void paint(Graphics g, int width, boolean paintbutton, int basenotepitch1, int basenotepitch2, int basenotepitchb1, int basenotepitchb2, int pitchcourant0,
        ...
        for (int i = 0; i < whiteKeys.size(); i++) {
            ...
            if (key.isNoteOn()) {
                g2.setColor(jfcBlue);
                g2.fill(key);
            } else if (!iscustom && paintbutton & ((key.kNum <= basenotepitch1 & key.kNum >= basenotepitch2) | (key.kNum <= basenotepitchb1 & key.kNum >= basenotepitchb2))) {
                g2.setColor(cg);
                g2.fill(key);
